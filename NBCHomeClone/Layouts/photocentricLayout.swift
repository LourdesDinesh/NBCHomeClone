...
class photocentricLayout : UICollectionViewLayout {
    ...
    override func prepare() {
        if cache.isEmpty {
            let columnWidth = width / CGFloat(noOfcolums)
            ...
            for item in 0..<collectionView!.numberOfItems(inSection: 0){
                ...
                let height = delegate.collectionview(collectionview: collectionView!, heightAtindexpath: indexPath)
                
                let frame = CGRect(x: xOff[column], y: yOff[column], width: columnWidth, height: height)
